...
try {
    ...
    ac('init', 'cjnlspjc5x8k20fn2ehrpm0b7', {datacenter: 'us1'});
    ac('debug');
    ac('pageview');
}
catch(e){
...
var webDemo = (function ($) {
    ...
    var _initPage = function(){
        ...
        if (typeof _gt != 'undefined' && _gt) {
            var title = $(".wiz-page")[$(".wiz-page").length-1].title; // the selector search rtuens two elements - old page[0] and new page[1]. For the starting page - we have one element only
            _gt.push(['config', {
                page: {
                    url: location.href,
                    title: title || pageId
        ...
        $('[data-gkc]').typeahead({
            ...
            source: function (query, process) {
                ...
                _genesys.widgets.bus.command("KnowledgeCenterService.getSuggestions", {"query": query, size: 4})
            ...
            afterSelect: function(selected){
                _genesys.widgets.bus.command('Search.open', {query: selected});
            }
        });
                            ...
                            $.get( DEMO_HOST + "/pod-version.txt")
                                ...
                                .always(function(){
                                    ...
                                    $.getScript( "//demo.genesyslab.com/gdemo_mobile/web/v2/js/cx-speechstorm.js", 
                                        function(){console.log("Speechstorm script is loaded");}
                                         );
                                    widgetCX.initCfg();
                                    if (CXW_ENABLED) {
...
// set env host
    var setEnvHost = function(demo_host){
        ...
        if (getURLParameter("host") != "null") {
           demo_host = "//" + getURLParameter("host");
        }
...
// Chat Widge processing -----
    ...
    var _openChatPopup = function (gmsDataId, isWidget, callback) {
        ...
        userData.subject = customParams.subject || "Web Chat";
        ...
        if (isWidget) {
            ...
            if (userData["_$_async_chat"] == "true"){
                
                userData.GCTI_Chat_AsyncMode = "true";
                userData.Chat_Async_RoutingTimeout = "1";      // decrease routing time to 5 secs (from 120 secs)
                userData.Chat_Async_WorkflowDebug = "false";   // debug messages to chat

                userData.async_chat = "bot";  // for SL customization

                // * using ChatKeeper notifications
                //widParams.push_notification_debug = "true";

                userData.mobile_token = genWeb.deviceToken;   
                userData.mobile_os = 'fcm';   
                userData.cxw_url = location.href;   

                /*userData.push_notification_deviceid = genWeb.deviceToken;   
                userData.push_notification_type = 'fcm';   
                userData.push_notification_provider = 'gdemo';   
                */
            }
...
// check if to show th eoriginal image size
    ...
    var dataForVideo = function(){
        ...
        params['demoID'] = TEMPLATE;
        params['companyTitle'] = genWeb.appName;
        params['PFS_id'] = local_number;
        ...
        if (typeof customUserData === "object") {
            ...
            if (pageParams.length > 0) {
                params["dynamicCaseData"] =
                    "<a href=\"" + genWeb.customer.IwsPageUrl + TEMPLATE + ".html?" + pageParams + "title=Show+Details\" />";
            }
        }
...
// ----------------- Open GKC Page ---------------
    ...
    var _getGkcDB = function(){
        return gkcDB;
    };

    var _getPodVersion = function(){
        return podVersion || "18-00-00.0";
    };

    // public methods
    return {
        getPodVersion: _getPodVersion,
        getGkcDB: _getGkcDB,
